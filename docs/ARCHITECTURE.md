...
Route 与任务阶段的事实源地位，也不在 Core 中绑定单一宿主。当前已完成 Phase 0-6；动态
Workflow 作者入口只生成和校验声明式 IR，Adaptive Runtime 只在 Checkpoint 间启动批准的子 Run。
...
[`DYNAMIC-WORKFLOW-AUTHORING.md`](./DYNAMIC-WORKFLOW-AUTHORING.md)，自适应运行时见
[`ADAPTIVE-WORKFLOW-RUNTIME.md`](./ADAPTIVE-WORKFLOW-RUNTIME.md)。未标记完成并通过契约测试的后续阶段能力
