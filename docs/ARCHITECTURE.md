...
Workflow IR、可替换 Agent Executor、持久化 Execution Journal、有界只读并发、原生宿主
Adapter 和独立进程协议；它不改变现有 Route 与任务阶段的事实源地位，也不在 Core 中绑定
单一宿主。当前只完成 Phase 0-3，写入 Agent/Worktree 和动态 Workflow 作者体验仍是 Proposed。

目标架构、不变量、公共契约边界、风险控制和分阶段退出标准见
[`MULTI-AGENT-EXECUTION-KERNEL.md`](./MULTI-AGENT-EXECUTION-KERNEL.md)，宿主接入见
[`EXECUTOR-PORTABILITY.md`](./EXECUTOR-PORTABILITY.md)。未标记完成并通过契约测试的后续阶段能力
